...
如果首次启动被 macOS 拦截,请移除 quarantine 属性后再打开:

```bash
xattr -dr com.apple.quarantine /Applications/yapyap.app
```

> Homebrew 已经不再支持 `--no-quarantine`,所以不能再使用 `brew install --cask --no-quarantine ...`。如果你把 app 安装到了别的目录,把上面的 `/Applications/yapyap.app` 换成实际路径即可。
...
yapyap 按需加载本地模型,当你切回在线模式时会主动释放本地模型占用的内存(尤其是 2 GB 量级的本地 Qwen),不需要重启 app,也不会让内存无限上涨。

在 Apple Silicon 上实测的大致占用:

| 配置 | 内存占用 |
|------|---------|
| 纯在线(豆包 + 在线后处理) | ~350 MB |
| 本地 ASR(SenseVoice Small) + 在线后处理 | ~360 MB |
| 本地 ASR + 本地 Qwen3 4B 后处理 | ~2.5 GB |

- 本地 ASR 模型(尤其 SenseVoice Small,~155 MB)增量很小,基本可以当作"免费"
- 本地 Qwen3 4B 后处理会额外占用约 2 GB(权重驻留在 Apple Silicon 的统一内存里)
- 任何时候从本地模式切回在线,都不需要重启 app
